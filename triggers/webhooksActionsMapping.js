...
const resourcesOperations = {
  ...
  'received_documents.e_invoices': {
    getOperation: ReceivedDocumentsApi.getPendingReceivedDocument,
    listOperation: ReceivedDocumentsApi.listPendingReceivedDocuments,
    resourceKeyId: 'document_id',
  },
};
...
module.exports = {
  getWebhooksOperation: (resource) => {
    if (resource.split('.')[0] == 'issued_documents') {
      resource = 'issued_documents';
    } else if (resource.split('.')[0] == 'received_documents') {
      if (resource == 'received_documents.e_invoices') {
        // keep the specific value for e-invoices
        resource = 'received_documents.e_invoices';
      } else {
        resource = 'received_documents';
      }
    }
  ...
  getDefaultGetOperationParams: (resource) => {
    ...
    if (resource.split('.')[0] == 'issued_documents') {
      ...
      defaults['type'] =
        res && IssuedDocumentType.fields('').choices.includes(res) ? res : 'invoice';
    } else if (resource.split('.')[0] == 'received_documents') {
      if (resource == 'received_documents.e_invoices') {
        defaults['type'] = 'agyo';
      } else {
        const res = resource.split('.')[1]?.slice(0, -1);
        defaults['type'] =
          res && ReceivedDocumentType.fields('').choices.includes(res) ? res : 'expense';
      }
