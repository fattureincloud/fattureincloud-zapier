...
const performSubscribe = async (z, bundle) => {
  ...
  bundle.inputData['data.types'] = bundle.inputData.event_types.map(
    (et) => `it.fattureincloud.webhooks.${bundle.inputData.resource}.${et}`
  );
  ...
  operation: {
    ...
    inputFields: [
      ...
      },
      {
        ...
        choices: [
          ...new Set(
            EventType.fields('').choices.map((type) => extractResourceAndOperation(type).resource)
          ),
        ],
        required: true,
