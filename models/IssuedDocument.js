...
module.exports = {
  fields: (prefix = '', isInput = true, isArrayChild = false) => {
    ...
    return [
      ...
      },
      {
        ...
        children: IssuedDocumentItemsListItem.fields(
          `${keyPrefix}items_list${!isInput ? '[]' : ''}`,
          isInput,
          true
        ),
      },
      {
        ...
        children: IssuedDocumentPaymentsListItem.fields(
          `${keyPrefix}payments_list${!isInput ? '[]' : ''}`,
          isInput,
          true
        ),
      },
  ...
  mapping: (bundle, prefix = '') => {
    ...
    return {
      ...
      payment_method: utils.removeIfEmpty(
        PaymentMethod.mapping(bundle, `${keyPrefix}payment_method`)
      ),
      ...
      ei_other_withholding_tax_type:
        bundle.inputData?.[`${keyPrefix}ei_other_withholding_tax_type`],
      ei_other_withholding_tax_causal:
        bundle.inputData?.[`${keyPrefix}ei_other_withholding_tax_causal`],
      items_list: utils.childMapping(
        bundle.inputData?.[`${keyPrefix}items_list`],
        `${keyPrefix}items_list`,
        IssuedDocumentItemsListItem
      ),
      payments_list: utils.childMapping(
        bundle.inputData?.[`${keyPrefix}payments_list`],
        `${keyPrefix}payments_list`,
        IssuedDocumentPaymentsListItem
      ),
      template: utils.removeIfEmpty(DocumentTemplate.mapping(bundle, `${keyPrefix}template`)),
      delivery_note_template: utils.removeIfEmpty(
        DocumentTemplate.mapping(bundle, `${keyPrefix}delivery_note_template`)
      ),
      acc_inv_template: utils.removeIfEmpty(
        DocumentTemplate.mapping(bundle, `${keyPrefix}acc_inv_template`)
      ),
      ...
      amount_withholding_tax_taxable:
        bundle.inputData?.[`${keyPrefix}amount_withholding_tax_taxable`],
      amount_other_withholding_tax: bundle.inputData?.[`${keyPrefix}amount_other_withholding_tax`],
      amount_other_withholding_tax_taxable:
        bundle.inputData?.[`${keyPrefix}amount_other_withholding_tax_taxable`],
      amount_enasarco_taxable: bundle.inputData?.[`${keyPrefix}amount_enasarco_taxable`],
      extra_data: utils.removeIfEmpty(
        IssuedDocument_extra_data.mapping(bundle, `${keyPrefix}extra_data`)
      ),
      ...
      ei_raw: utils.jsonFieldToObject(
        bundle.inputData?.[`${keyPrefix}ei_raw`],
        `${keyPrefix}ei_raw`
      ),
